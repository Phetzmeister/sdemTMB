...
update_diffterms = function(self, private) {
  ...
  for (i in 1:length(private$sys.eqs.trans)) {
    private$diff.terms[[i]] = lapply(private$diff.processes, function(x) Deriv(private$sys.eqs.trans[[i]]$rhs, x, cache.exp=FALSE))
...
compile_cppfile = function(self, private) {
  ...
  if (private$compile) {
    ...
    try(dyn.unload(dynlib(private$cppfile.path)),silent=T)
    try(dyn.load(dynlib(private$cppfile.path)),silent=T)
  }
  ...
  if (!private$compile) {
    ...
    if (.Platform$OS.type=="unix") {
      ...
      if (!file.exists(modelpath.so)) {
        message("There does not exist a pre-compilation of '",private$modelname,"' in directory ",private$cppfile.directory,". Compiling...")
  ...
  if (!private$compile) {
    message("Loading pre-compiled model. Use $set_compile(TRUE) to recompile.")
    try(dyn.load(dynlib(private$cppfile.path)),silent=T)
  }
