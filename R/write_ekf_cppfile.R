...
vector<Type> get_free_pars__(vector<int> mapints, int sum_mapints, vector<Type> parvec) {
  ...
  for(i in 1:private$n){
    terms = lapply(private$state.names,function(x) hat2pow(Deriv(private$diff.terms[[i]]$dt,x=x,cache.exp=FALSE)))
  ...
  # txt = c(txt, "\t vector<vector<Type>> F_all(k__);")
  ...
  txt = c(txt, sprintf("\t\t\t h__  = hfun__(%s);",paste(hvars2,collapse=", ")))
