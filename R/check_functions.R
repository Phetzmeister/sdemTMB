...
check_parameters <- function(parmat, self, private) {
  ...
  # are there duplicates in the provided matrix?
  if (length(unique(parnames)) != length(parnames)) {
    warning("There are duplicate parameter entries. Last entry will be used.")
  }

  # does the parameter name already exist?
  if (any(parnames %in% names(private$parameters))) {
    # warning("Overwriting the parameters:\n\t",
    #         paste(parnames[parnames %in% names(private$parameters)],collapse=", "))
  }
