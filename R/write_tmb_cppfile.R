...
vector<Type> get_free_pars__(vector<int> mapints, int sum_mapints, vector<Type> parvec) {
  ...
  for(i in 1:length(private$state.names)){
    fvars2 = sub(pattern=sprintf("^%s$",state.names[i]), replacement=sprintf("%s(Ncumsum__(i)+j)",state.names[i]), x=fvars2)
  }
  ...
  state.names = private$state.names
  for(i in 1:length(private$state.names)){
    gvars2 = sub(pattern=sprintf("^%s$",state.names[i]), replacement=sprintf("%s(Ncumsum__(i)+j)",state.names[i]), x=gvars2)
  }
  ...
  state.names = private$state.names
  for(i in 1:length(private$state.names)){
    obsvars2 = sub(pattern=sprintf("^%s$",state.names[i]), replacement=sprintf("%s(Ncumsum__(i))",state.names[i]), x=obsvars2)
  }
  ...
  txt = c(txt, "\n\t ////////////////////////////////////////////////////////")
  txt = c(txt, "\t //////////// MAIN FOR-LOOP OVER STATES ///////////")
  txt = c(txt, "\t ////////////////////////////////////////////////////////")
  ...
  txt = c(txt, sprintf("\t\t\t Xi__ << %s;",paste(private$state.names,"(Ncumsum__(i)+j)",collapse=", ",sep="")))
  txt = c(txt, sprintf("\t\t\t Xip1__ << %s;",paste(private$state.names,"(Ncumsum__(i)+j+1)",collapse=", ",sep="")))
